...
export default function CommissionTable() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center py-6 px-3 sm:py-8 sm:px-6 lg:px-10">
      <div className="w-full max-w-6xl bg-white shadow-lg rounded-2xl overflow-hidden relative">
        {/* Overlay to prevent selection */}
        <div className="absolute inset-0 z-10 bg-transparent"></div>

        <div className="px-4 sm:px-6 py-4 bg-purple-600 rounded-t-2xl text-center relative z-20">
          <h1 className="text-lg sm:text-2xl md:text-3xl font-bold text-white">
            Payout Structure
          </h1>
        </div>

        <div className="px-4 sm:px-6 py-3 sm:py-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-800 text-xs sm:text-sm md:text-base relative z-20">
          ⚠️ This Payout Structure is valid for 6 months, effective from 1st October 2025.
        </div>

        <div className="overflow-x-auto relative z-20">
          <table className="min-w-full divide-y divide-gray-200 text-xs sm:text-sm md:text-base table-auto">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-3 sm:px-6 py-2 text-left font-medium text-gray-600 uppercase tracking-wider break-words w-1/2">
                  Category
                </th>
                <th className="px-3 sm:px-6 py-2 text-left font-medium text-gray-600 uppercase tracking-wider break-words">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {commissionData.map((item, index) => (
                <tr key={index} className="hover:bg-gray-50 transition">
                  <td className="px-3 sm:px-6 py-2 text-gray-900 break-words whitespace-normal">{item.category}</td>
                  <td className="px-3 sm:px-6 py-2 text-gray-700 whitespace-normal">{item.businessReg || "-"}</td>
                  <td className="px-3 sm:px-6 py-2 text-gray-700 whitespace-normal">{item.drivePurchase || "-"}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="px-4 sm:px-6 py-5 mt-6 bg-gray-50 border-t border-gray-200 text-gray-700 text-xs sm:text-sm md:text-base rounded-b-2xl relative z-20">
          ...
          <ol className="list-decimal pl-5 space-y-3 leading-relaxed">
            ...
            <li>
              <span className="font-medium text-gray-900">Business Month Definition:</span> Business month follows the
            ...
            <li>
              ...
              <span className="font-medium"> 7th Date of the Next Business Month</span>, except on <span className="italic">holidays, Saturdays, and Sundays</span>.
            </li>

            <li>
              <span className="font-medium text-gray-900">Account Inactivity:</span> An account will be marked as <span className="font-medium">inactive</span> if:
