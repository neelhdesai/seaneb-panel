...
export const createUser = async (req, res) => {
  try {
    ...
    // Default to consultant if no role passed
    const user = new User({
      name,
      email,
      mobileNumber,
    ...
    if (error.code === 11000) {
      const duplicateField = Object.keys(error.keyPattern)[0]; // mobileNumber, email, or consultantPan
      let fieldName = "";
...
export const denyUser = async (req, res) => {
  try {
    ...
    user.deniedReason = reason || "Not specified";
    ...
    await sendEmail(
      ...
      denialTemplate(user.name, reason)
    );
